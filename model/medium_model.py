...
class MediumModel(torch.nn.Module):
    def __init__(self, dataset_name):
        ...
        self.linear2 = torch.nn.Linear(6, len(self.generator.palette))
    ...
    def save(self):
        path = f'./trained_models/medium/{self.name}'
        os.makedirs(os.path.dirname(path), exist_ok=True)
        torch.save(self.state_dict(), f'{path}')

    @classmethod
    def load(cls, dataset_name):
        path = f'./trained_models/medium/{dataset_name}'
        model = MediumModel(dataset_name)
        try:
